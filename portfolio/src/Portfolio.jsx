...
const skillsData = 'RAG Systems / Strands Agents / LangChain / LangGraph / Transformers / PyTorch / TensorFlow / scikit-learn / Pandas / NumPy / Python / Java / C++ / C / SQL / FastAPI / .NET / Spring Boot / Django / PostgreSQL / TimescaleDB / Supabase / Docker / Azure / AWS / Git / ';
const skillGroups = [
...
const systems = [
  ['A', 'Pipeline', 'A direct, auditable path: MedGemma triage → SAM3 lesion segmentation → task-specific CNN → BiomedCLIP re-ranking.', ['MedGemma', 'SAM3', 'CNN', 'BiomedCLIP'], 'Fixed order, nothing to negotiate'],
  ['B', 'Debate', 'The same specialists become advocates. They make their cases to a judge agent, which weighs their evidence and reaches a diagnosis.', ['Advocates', 'Evidence', 'Judge'], 'Specialists argue, a judge decides'],
  ['C', 'Agent forest', 'Role-specialised MedGemma agents independently assess the scan; a majority vote produces a transparent, robust result.', ['Agent 1', 'Agent 2', 'Agent N', 'Vote'], 'No coordinator, just a majority'],
];
...
function Portfolio() {
  ...
  useEffect(() => {
    const updateSystem = () => {
      ...
      const lead = section.querySelector('.systems-intro')?.offsetHeight ?? 0;
    ...
    updateSystem();
    window.addEventListener('scroll', updateSystem, { passive: true });
    return () => window.removeEventListener('scroll', updateSystem);
  }, []);
  ...
  return <div className="portfolio-redesign">
    ...
    <header className="site-header"><div className="shell nav-shell"><a href="#top" onClick={(event) => scrollToSection(event, 'top')} className="brand">Tamara Kostova</a><nav><a href="#systems" onClick={(event) => scrollToSection(event, 'systems')}>Current project</a><a href="#work" onClick={(event) => scrollToSection(event, 'work')}>Work</a><a href="#projects" onClick={(event) => scrollToSection(event, 'projects')}>Projects</a><a href="#stack" onClick={(event) => scrollToSection(event, 'stack')}>Skills</a><a href="#research" onClick={(event) => scrollToSection(event, 'research')}>Research</a><a href="#education" onClick={(event) => scrollToSection(event, 'education')}>Education</a><a href="#contact" onClick={(event) => scrollToSection(event, 'contact')}>Contact</a><Link to="/about">About</Link></nav><div className="nav-socials"><a href="https://github.com/tamara-kostova" target="_blank" rel="noreferrer" aria-label="GitHub"><FaGithub /></a><a href="https://www.linkedin.com/in/tamara-kostova/" target="_blank" rel="noreferrer" aria-label="LinkedIn"><FaLinkedin /></a></div></div></header>
    <main id="top">
      <section className="hero shell"><em className="hero-eyebrow">I build</em><h1><span>AI systems</span><span>that do</span><i>real work</i></h1><div className="hero-bottom"><p>Multi-agent systems, medical AI pipelines and RAG. <br></br>Research prototypes through to software running in production.<br></br>Software engineer at ITQuarks, MSc Data Science at FCSE.</p><div className="hero-links"><a href="https://github.com/tamara-kostova" target="_blank" rel="noreferrer">GitHub ↗</a><a href="https://www.linkedin.com/in/tamara-kostova/" target="_blank" rel="noreferrer">LinkedIn ↗</a><a href="mailto:tamarakostova.bt@gmail.com">Email ↗</a></div></div></section>
      <section id="systems" className="systems">
        <div className="shell systems-intro">
          <div className="systems-heading"><h3>Current project</h3><em>MSc thesis · FCSE Skopje</em></div>
          ...
          <em className="systems-cue">Keep scrolling — the diagram builds each one ↓</em>
        </div>
        <div className="shell systems-inner">
          ...
          <div className="systems-grid"><SystemDiagram nodes={active[3]} /><div><div className="system-title"><strong>{active[0]}</strong><h3>{active[1]}</h3></div><p>{active[2]}</p><div className="system-actions">{systems.map((item, i) => <button className={i === system ? 'selected' : ''} onClick={() => setSystem(i)} key={item[0]} aria-label={`Show system ${item[0]}: ${item[1]}`}>{item[0]}</button>)}<a className="pill" href="https://github.com/tamara-kostova/MultiAgentMedClassifier" target="_blank" rel="noreferrer">Repository ↗</a></div></div></div>
        </div>
      </section>
      ...
      <section id="projects" className="shell projects"><div className="section-heading"><h2>Projects</h2><em>Selected work first — everything else below</em></div><div className="project-list">{projectsData.map(([title, tag, description, link], i) => <a href={link} target="_blank" rel="noreferrer" key={title}><small>{String(i + 1).padStart(2, '0')}</small><div><h3>{title}</h3><em>{tag}</em></div><p>{description}</p><b>↗</b></a>)}</div><div className="more-projects"><em>Other Projects:</em><div className="more-projects-list">{moreProjectsData.map(([title, tag, link]) => <a href={link} target="_blank" rel="noreferrer" key={title}><span>{title}</span><small>{tag}</small></a>)}</div><a className="more-projects-all" href="https://github.com/tamara-kostova" target="_blank" rel="noreferrer">All on GitHub ↗</a></div></section>
...
function SystemDiagram({ nodes }) {
  const system = nodes[0] === 'Advocates' ? 1 : nodes[0] === 'Agent 1' ? 2 : 0;
  const graph = graphLayouts[system];
  return <svg className="system-diagram" viewBox="0 0 100 60" role="img" aria-label="System architecture diagram">
